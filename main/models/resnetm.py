...
__all__ = ['ResNetM', 'BasicBlock', 'Bottleneck']  # model_registry will add each entrypoint fn to this
...
default_cfgs = {
    ...
    'resnet14t': _cfg(
        ...
        test_crop_pct=0.95, test_input_size=(3, 224, 224),
        first_conv='conv1.0'),
    'resnet18m': _cfg(url='https://download.pytorch.org/models/resnet18-5c106cde.pth'),
    'resnet18d': _cfg(
...
class ResNetM(nn.Module):
    ...
    def __init__(
        ...
        self.global_pool, self.fc = create_classifier(self.num_features*8, self.num_classes, pool_type=global_pool)
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            x = self.layer3(x)
            x = self.layer4(x)
            x = x.reshape(-1,x.shape[1]*8,x.shape[2],x.shape[3])
        return x
...
def _create_resnet(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(ResNetM, variant, pretrained, **kwargs)
...
@register_model
def resnet18m(pretrained=False, **kwargs):
