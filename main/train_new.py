...
parser.add_argument('--model-ema-decay', type=float, default=0.99,
...
parser.add_argument('--report-metrics', default=['loss', 'f1', 'kappa','fk'],
                    nargs='+', choices=['acc', 'f1', 'recall', 'precision', 'kappa','fk'],
...
def main():
    ...
    dataset_train = MultiPhaseLiverDataset2d(args, is_training=True)

    dataset_eval = MultiPhaseLiverDataset2d(args, is_training=False)
    ...
    if args.rank == 0:
        ...
        for metric in args.report_metrics:
            ...
            decreasing = True if metric == 'loss' else False
        ...
        # second_best_metrics = {}
        # second_metric_savers = {}
        # for metric in args.report_metrics:
        #     second_best_metrics[metric] = {'value': None, 'epoch': None}
        #     second_metric_savers[metric] = CheckpointSaver(
        #         model=model, optimizer=optimizer,
        #         args=args, model_ema=model_ema,
        #         amp_scaler=loss_scaler,
        #         checkpoint_prefix=f'second_{metric}_checkpoint',
        #         checkpoint_dir=output_dir,
        #         recovery_dir=output_dir,
        #         decreasing=decreasing,
        #         max_history=args.checkpoint_hist)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if metric_savers is not None:
                ...
                for metric in args.report_metrics:
                    if best_metrics[metric]['value'] is None or (
                            eval_metrics[metric] > best_metrics[metric]['value']):
                        best_metrics[metric]['value'] = eval_metrics[metric]
                        best_metrics[metric]['epoch'] = epoch
                    ckpt_saver = metric_savers[metric]
                    ckpt_saver.save_checkpoint(epoch, metric=eval_metrics[metric])

            # if second_metric_savers is not None:
            #     # Save the best checkpoint for this metric
            #     for metric in args.report_metrics:
            #         if second_best_metrics[metric]['value'] is None or (
            #                 eval_metrics[metric] > second_best_metrics[metric]['value'] and eval_metrics[metric] <
            #                 best_metrics[metric]['value']):
            #             second_best_metrics[metric]['value'] = eval_metrics[metric]
            #             second_best_metrics[metric]['epoch'] = epoch
            #             ckpt_saver = second_metric_savers[metric]
            #             ckpt_saver.save_checkpoint(epoch, metric=second_best_metrics[metric]['value'])

            # if metric_savers is not None:
            #     # Save the best checkpoint for this metric
            #     for metric in args.report_metrics:
            #         if best_metrics[metric]['value'] is not None and (eval_metrics[metric] > best_metrics[metric]['value']):
            #             if second_best_metrics[metric]['value'] is None or (best_metrics[metric]['value'] > second_best_metrics[metric]['value']):
            #                 cmd = 'mv %s/best_%s_checkpoint-%s.pth.tar %s/second_%s_checkpoint-%s.pth.tar' % (
            #                     metric_savers[metric].checkpoint_dir, metric, best_metrics[metric]['epoch'],
            #                     metric_savers[metric].checkpoint_dir, metric, best_metrics[metric]['epoch'])
            #                 os.system(cmd)
            #                 cmd = 'rm -rf %s/second_%s_checkpoint-%s.pth.tar' % (
            #                     second_metric_savers[metric].checkpoint_dir, metric, second_best_metrics[metric]['epoch'])
            #                 os.system(cmd)
            #                 second_best_metrics[metric]['value'] = best_metrics[metric]['value']
            #                 second_best_metrics[metric]['epoch'] = best_metrics[metric]['epoch']
            #             best_metrics[metric]['value'] = eval_metrics[metric]
            #             best_metrics[metric]['epoch'] = epoch
            #             ckpt_saver = metric_savers[metric]
            #             ckpt_saver.save_checkpoint(epoch, metric=best_metrics[metric]['value'])
            #         elif best_metrics[metric]['value'] is None:
            #             best_metrics[metric]['value'] = eval_metrics[metric]
            #             best_metrics[metric]['epoch'] = epoch
            #             ckpt_saver = metric_savers[metric]
            #             ckpt_saver.save_checkpoint(epoch, metric=best_metrics[metric]['value'])
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            input = input.reshape(-1, 8, input.shape[2], input.shape[3], input.shape[4])
            output = model(input)
